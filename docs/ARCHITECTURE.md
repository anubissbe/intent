...
## Source-control connections and Git credentials

The daemon resolves each repository's provider from its origin and a registry keyed by canonical full instance URL. GitHub and multiple GitLab instances coexist. Workspace PR monitoring has per-connection rate-limit state and caches. A picker selection never changes the daemon's provider for another repository.

Each stored GitLab connection secret is an atomic `{ provider, instanceUrl, token }` record, under a separate account derived from the instance ID. Resolution verifies the embedded binding. GitHub retains its dedicated secret slot for device-flow compatibility. Legacy GitLab host-bound records remain readable, and explicit registry entries or disconnect tombstones override legacy settings. The historical `github.*` RPCs continue to address GitHub; neutral `sourceControl.*` calls supply connection/repository context. See [the wire contract](protocol/methods/integrations.md).

Daemon-internal Git operations carry a typed credential containing its HTTPS origin, port, installation prefix and username. Operations with an Intent credential use bounded system Git and disable redirects, including narrower URL-specific redirect settings. This avoids libgit2 reusing a credential across a same-host redirect without another callback. Shell Git receives an instance-scoped helper ahead of existing helpers, with those helpers retained as fallbacks. Only the short-lived Git child carries the token (`INTENT_GIT_SCOPED_PASSWORD`); argv and helper configuration contain no secret. Local and SSH operations retain their existing transport.

Terminals and agents instead receive `intentd git-credential` helpers scoped to registered instances, with `credential.useHttpPath=true`. No token enters their environment. On every `get`, the helper forwards protocol, host, path and optional username to `system.gitCredential` over the Unix socket. The daemon validates the registry binding and per-connection child-credential setting. A disconnect therefore affects the next request in an already-open terminal. Missing credentials, scope mismatches, a stopped daemon and `store`/`erase` remain silent misses so other Git helpers can run.

The credential RPC is UDS-only. Remote clients use the normal Git RPCs; WSS cannot read raw credentials. Audit logs record grant/denial and requesting pid without token values. SSH remains governed by the user's SSH configuration.
