...
The plaintext secret is stored beside its hash (migration `0125`) so the owner can copy an open invite's link again: every `list` row rebuilds `url` from the stored secret through the same envelope `create` used, so it is byte-identical to the minted link. `url` is **absent, never an error**, when the row predates the stored secret or when no link can be built at read time (listener down, no dialable route) — a `list` whose envelope fails to resolve still answers every open row, just without `url`. The envelope is resolved once per call, not per row. On `create`, the one link formatted from the single pre-mint envelope is stamped as both `invite.url` and the top-level `url`, so the two are the same string by construction.
...
Three daemon-owned `sharing.*` settings (§5.12; TOML-backed under a `[sharing]` config.toml section) bound how many guests a workspace admits and how many WSS connections guests may hold. A **guest** is any non-primary principal; the owner (legacy token) and the UDS path are never counted against any of them.
...
| HTTP 401 / 503 | — | Upgrade gate: unknown or revoked bearer token on `/ws` / `/tunnel`; `/invite` at its 32-connection cap; a guest's `/ws` upgrade while `sharing.maxGuestConnections` or `sharing.maxConnectionsPerGuest` is spent |
