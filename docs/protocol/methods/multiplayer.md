...
- `sharing.maxGuestConnections` — number, default `40`; `0` = unlimited, else `1..=100000`. Sized at listener construction from the boot settings — a change applies on **daemon restart**.
- `sharing.maxConnectionsPerGuest` — number, default `4`; `0` = unlimited, else `1..=1000`. Same restart rule as above.
...
Owner-only (the 31 refused types): `terminal:data` / `terminal:exit` (raw PTY bytes), `host:exec:stdout` / `stderr` / `exit`, `script:changed` / `output` / `state`, `browser:tab-opened` / `tab-updated` / `tab-closed`, `client:connected` / `client:disconnected`, `hook:run-started` / `hook:run-completed` (hook code and carried state), `agent:permission:request` / `agent:permission:resolved`, `workspace:transfer:progress` / `ready` / `failed`, `git:clone:progress` / `git:clone:done`, `gitRoot:registered` / `updated` / `unregistered`, `app:ui-highlight` / `app:ui-navigate` / `app:workspace-open`, `settings:changed`, `github:auth-changed`, `mcp.servers:status-changed`.
...
| HTTP 401 / 503 | — | Upgrade gate: unknown or revoked bearer token on `/ws` / `/tunnel`; `/invite` at its 32-connection cap; a per-principal credential's `/ws` upgrade while `sharing.maxGuestConnections` or `sharing.maxConnectionsPerGuest` is spent |
