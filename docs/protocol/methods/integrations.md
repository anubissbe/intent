...
| github.identityProof.create | nonce (req), hostLabel (req) | { gistId, login } — the **guest side** of the multiplayer join (§5.47 "First-time guest"; [intent-hq/intentd#1963](https://github.com/intent-hq/intentd/pull/1963)): publishes a **secret** gist (`POST /gists`, `public: false`) whose single file `intent-join-proof.txt` carries `nonce` on its first line and a human note naming `hostLabel` on the second (description `Intent identity proof for <hostLabel> (safe to delete)`), then answers the gist id and the token owner's `login`. Uses **only the stored** `github.connect` token — the env / `gh` fallbacks are never used — and first confirms via `GET /user`'s `X-OAuth-Scopes` that it grants `gist`. Both params are trimmed, non-empty single lines (control characters → `-32602`). **Administrator-only** (service gate; not on the collaborator allowlist). Failures: no (or rejected) stored token `-32603 { code: "github-not-connected" }`, token without `gist` `-32603 { code: "github-scope-missing" }`, GitHub unreachable / server error `-32603 { code: "github-unreachable" }`. The token never crosses the wire |
| github.identityProof.delete | gistId (req) | { ok: true } — deletes a proof gist made by `identityProof.create` (`DELETE /gists/{gistId}`); **idempotent**: an already-deleted gist (`404`) is `ok`. `gistId` must be non-empty alphanumeric (`-32602`). Same token rule, administrator gate and `-32603` codes as `create` |
