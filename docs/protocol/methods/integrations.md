...
> The device flow requests the scopes **`repo`, `read:org`, `workflow`, `gist`** by default; `gist`
> was added for the multiplayer join proof (`github.identityProof.create`, §5.47), so a token
> connected **before** it was added does not carry it — `github.revoke` + `github.connect` again
> (reauthorize) before a first proof; `github.authStatus.isConfigured` alone does **not** establish
> gist permission (it validates the token, not its scopes — only `identityProof.create`'s own
> `X-OAuth-Scopes` check does).
>
...
> - **User lookup** for the collaborator picker: `github.users.search` (v10.3) is a login-prefix
...
| github.connect | — | { ok: true, userCode, verificationUri, expiresIn, interval } — starts the OAuth **device flow** (or returns the SAME codes while one is pending — idempotent), requesting the default scopes `repo`, `read:org`, `workflow`, `gist` (a token connected before `gist` joined the list must be reauthorized before `github.identityProof.create` can succeed). The daemon polls GitHub in the background; terminal transitions arrive as `github:auth-changed` events (§6.5). A missing/empty `sourceControl.github.oauthClientId` or an unreachable login host → `-32603` |
...
| github.identityProof.create | nonce (req), hostLabel (req) | { gistId, login } — the **guest side** of the multiplayer join (§5.47 "First-time guest"; [intent-hq/intentd#1965](https://github.com/intent-hq/intentd/pull/1965)): publishes a **secret** gist (`POST /gists`, `public: false`) whose single file `intent-join-proof.txt` carries `nonce` on its first line and a human note naming `hostLabel` on the second (description `Intent identity proof for <hostLabel> (safe to delete)`), then answers the gist id and the token owner's `login`. Uses **only the stored** `github.connect` token — the env / `gh` fallbacks are never used — and first confirms via `GET /user`'s `X-OAuth-Scopes` that it grants `gist`. Both params are trimmed, non-empty single lines (control characters → `-32602`). **Administrator-only** (service gate; not on the collaborator allowlist). Failures: no (or rejected) stored token `-32603 { code: "github-not-connected" }`; the `X-OAuth-Scopes` check finding no `gist` `-32603 { code: "github-scope-missing" }`; a **transport** failure (connection refused / reset, DNS, TLS, timeout — GitHub could not be reached) `-32603 { code: "github-unreachable" }`; **any other** forge or decode failure — a GitHub HTTP error such as a `5xx` or `422`, a rate-limit refusal (`429`, or a `403` naming the rate limit), or an unparsable response — is a plain `-32603` with **no guaranteed discriminator**: most surface as `"Internal error"` with the cause as string `data`, but a rate-limit refusal keeps its own rate-limit `message` and carries **no `data` at all**, so clients need a generic fallback beside the three discriminators and must not assume `data` is present. The token never crosses the wire |
| github.identityProof.delete | gistId (req) | { ok: true } — deletes a proof gist made by `identityProof.create` (`DELETE /gists/{gistId}`); **idempotent**: an already-deleted gist (`404`) is `ok`. `gistId` must be non-empty alphanumeric (`-32602`). Same **stored-token-only** rule and administrator gate as `create`, but its own failure set: no (or rejected) stored token `-32603 { code: "github-not-connected" }`, a transport failure `-32603 { code: "github-unreachable" }`, any other forge / decode error a plain `-32603` with no guaranteed `data.code`. It performs **no scope check** and never emits `github-scope-missing` (a token that could create the gist can delete it) |
| github.users.search | query (req), limit? | { users: { id: number, login: string, avatarUrl: string, htmlUrl: string }[] } — login-prefix user search over `GET /search/users` (v10.3; [intent-hq/intentd#1912](https://github.com/intent-hq/intentd/pull/1912)), backing the collaborator picker. `query` is required (missing → `-32602`); it is trimmed, one leading `@` is dropped, and the remainder is sent as `q = "<query> in:login type:user"` so hits are user accounts matched on login. A **blank** query (empty after trimming / `@`-stripping) answers `{ users: [] }` **without a forge call**. `limit` is the page size: default **8**, clamped into **[1, 10]** (out-of-range values are clamped, not rejected — an exception to the namespace's `limit` default 50 / max 200 convention; no `nextToken`, a single page only). Every hit carries `id` (the FE keys the picker on it — a hit the forge answered without one is dropped), `login`, `avatarUrl` and `htmlUrl` (the latter two `""` when absent). **Administrator-only**: like the rest of `github.*` the name is not on the collaborator allowlist, so a per-principal (collaborator) wire caller is refused with `-32003 Forbidden` (`data { code: "forbidden", detail }`) before dispatch regardless of workspace roles, and the service layer re-applies the same administrator gate. Draws on GitHub's **search-API quota** (30 requests/min per token), so clients should debounce. Token / forge failures → `-32603` per the conventions above |
...
> **Auth model — personal API key (no OAuth).** A local
...
> **Auth model — token + org from the environment (no OAuth, no
