...
- `-32602 "presence.update: …"` on a malformed body: `focus` not an array, an item not an object, a missing / empty `focus[].workspaceId`, a non-string `agentId` / `noteId`, `typing` neither an object nor `null`, or a missing `typing.agentId`.
...
- Presence is a **client-connection** concern: every method resolves the principal from the connection's bound wire caller. Agent / daemon callers and an unbound request are refused with `-32003 Forbidden` (`data { code: "forbidden", detail }`, e.g. `"presence.update is only available to client connections"`).
