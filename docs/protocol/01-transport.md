...
- A plain HTTPS `GET /health` returns `{"status":"ok","clients":<n>,"guestConnections":<n>}` for liveness probing — `guestConnections` (additive, within 10.2; [intent-hq/intentd#1917](https://github.com/intent-hq/intentd/pull/1917)) is the number of connections currently admitted on per-principal credentials (the administrator/legacy token and the UDS path are never counted) against the `sharing.maxGuestConnections` / `sharing.maxConnectionsPerGuest` caps (§5.47 "Guest caps").
- The same listener also serves the non-JSON-RPC `/tunnel` WebSocket endpoint — the binary loopback port-forwarding surface (§1.4).
- Any path other than `/ws`, `/tunnel` and `/invite` (§2.4; the unauthenticated invite-redemption endpoint, within 10.2) is rejected at upgrade time with `404` (socket destroyed).
