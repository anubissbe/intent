...
| §5.47 Multiplayer — `principal.*` / `workspace.invite.*` / `invite.inspect` / `invite.accept` / `invite.challenge` / `invite.prove` / `workspace.members.*` (principals, invite links, the gist identity-proof join, membership, the collaborator allowlists) | [methods/multiplayer.md](./methods/multiplayer.md) |
